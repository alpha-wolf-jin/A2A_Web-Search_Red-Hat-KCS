...
def _init_sub_agent(
    ...
    *,
    ...
    """Connect to an A2A sub-agent. If optional is True, return None when unreachable."""
    ...
    try:
        ...
        if optional:
            ...
            print(f"  ! Skipping {label} at {host}:{port} — {exc}{detail}")
            return None
        raise
...
def _routing_instructions(*, web_available: bool, kcs_available: bool) -> str:
    ...
    if web_available:
        return (
            "Only the general web search specialist is available; the Red Hat KCS agent "
            "is not running or unreachable. Use the web search handoff for every question. "
            "If the user asks specifically for official Red Hat KCS or Access Red Hat "
    ...
    if kcs_available:
        return (
            "Only the Red Hat KCS specialist is available; general web search is not "
            "running or unreachable. Use the Red Hat knowledge base handoff for every "
            "question. For topics outside Red Hat, search the knowledge base for "
...
def build_concierge(
    ...
    *,
    verbose: bool = False,
    allow_missing_agents: bool = True,
) -> RequirementAgent:
    """Connect to web search and Red Hat KCS sub-agents and assemble the orchestrator.

    Each leaf URL is probed independently. **Every** reachable agent gets a handoff
    tool (both if both are up, one if only one is up). At least one must be
    reachable unless allow_missing_agents is False, in which case both must succeed.
    """
    print("Initializing sub-agents (each leaf is tried; all reachable agents are attached):")
    optional = allow_missing_agents
    web_agent = _init_sub_agent(host, web_search_port, "web_search", optional=optional)
    kcs_agent = _init_sub_agent(host, redhat_kcs_port, "red_hat_kcs", optional=optional)

    handoff_tools: list[HandoffTool] = []
    if web_agent is not None:
        assert web_agent.agent_card is not None
        handoff_tools.append(
            HandoffTool(
                target=web_agent,
                name=web_agent.name,
                description=(
                    f"{web_agent.agent_card.description}\n\n{_WEB_SEARCH_TOOL_DESC}"
                ),
            )
        )
    if kcs_agent is not None:
        assert kcs_agent.agent_card is not None
        handoff_tools.append(
            HandoffTool(
                target=kcs_agent,
                name=kcs_agent.name,
                description=(
                    f"{kcs_agent.agent_card.description}\n\n{_REDHAT_KCS_TOOL_DESC}"
                ),
            )
        )
    ...
    if not handoff_tools:
        sys.stdout.flush()
        raise RuntimeError(
            "No sub-agents are reachable. Start at least one leaf agent:\n"
            f"  - web-search-agent-deepseek.py on {host}:{web_search_port}\n"
            f"  - redhat-kcs-agent-deepseek.py on {host}:{redhat_kcs_port}"
        )

    lines = []
    if web_agent is not None:
        lines.append(f"{web_agent.name} @ {host}:{web_search_port}")
    if kcs_agent is not None:
        lines.append(f"{kcs_agent.name} @ {host}:{redhat_kcs_port}")
    print(f"  -> Concierge will use {len(lines)} leaf agent(s): " + " | ".join(lines))

    web_ok = web_agent is not None
    kcs_ok = kcs_agent is not None
    if web_ok and kcs_ok:
        concierge_desc = (
        ...
        concierge_desc = (
            "Concierge with general web search only (Red Hat KCS agent unavailable)."
        )
    else:
        concierge_desc = (
            "Concierge with Red Hat KCS only (general web search agent unavailable)."
        )
    ...
    concierge = ConfiguredRequirementAgent(
        ...
        instructions=_routing_instructions(web_available=web_ok, kcs_available=kcs_ok),
    )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--require-all-agents",
        action="store_true",
        help="Fail unless both leaf agents are up (default: attach all that are reachable, min 1)",
    )
